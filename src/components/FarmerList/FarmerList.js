...
const FarmerList = (props) => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const response = await axios.get("https://jml-coffeetalk-client.herokuapp.com/farmers");
  ...
  const handleCreateRoom = async event => {
    event.persist()
    try {
      const farmerResponse = await axios.get(`https://jml-coffeetalk-client.herokuapp.com/farmers/${event.target.id}`)
      const farmerData = await farmerResponse.data
      // await console.log(`https://jml-coffeetalk-client.herokuapp.com/${farmerData.farmerLocation}`)
  ...
  const weatherHit = async data => {
    try {
      const weatherResponse = await axios.get(`https://jml-coffeetalk-client.herokuapp.com/weather/${data.farmerLocation}`)
  ...
  const smsHit = async data => {
    const smsResponse = await axios.get(`https://jml-coffeetalk-client.herokuapp.com/sms/${data.phoneNumber}`)
